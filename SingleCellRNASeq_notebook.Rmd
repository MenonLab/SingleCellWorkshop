...
data1_filt <- ScaleData(data1_filt, features = all.genes)
data1_filt
```
...
```{r}
DimHeatmap(data1_filt, dims = 1:4, cells = 500, balanced = TRUE)
```
...
####Seurat offers several dimensional reduction approaches for visualization of the single-cell data, such as PCA, tSNE, and UMAP. The goal of these algorithms is to compress the variation in the data down to 2-D for visualization, preserving some aspects of local distance. Do not use tSNE and UMAP for clustering - there are multiple reports showing that the distortion when projecting into 2-D space can be extreme (see Lior Pachter's work). 
...
# ones
cluster0_markers <- FindMarkers(data1_filt, ident.1=3, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
```{r}
cluster10_markers <- FindMarkers(data1_filt, ident.1=10, ident.2=c(6,7), only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
head(cluster10_markers)
```

###Subsetting data into a new Seurat object
```{r}
keepcells = colnames(data1_filt)[which(data1_filt$seurat_clusters %in% c(6,7,10))]
